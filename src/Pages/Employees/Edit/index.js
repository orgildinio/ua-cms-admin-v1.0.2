...
const Edit = (props) => {
  ...
  useEffect(() => {
    if (props.employee) {
      const languageData = {
        name: !props.employee[cookies.language]
          ? ""
          : props.employee[cookies.language].name,
        about: !props.employee[cookies.language]
          ? ""
          : props.employee[cookies.language].about,

        degree: !props.employee[cookies.language]
          ? ""
          : props.employee[cookies.language].degree,
      };

      form.setFieldsValue({ ...props.employee, ...languageData });

      if (props.employee.positions && props.employee.positions.length > 0) {
        setCheckedKeys(() => {
          return props.employee.positions.map((cat) => cat._id);
        });
      }
      ...
      if (props.employee.picture) {
        setPicture(() => ({
          ...
          url: `${base.cdnUrl}/150x150/${props.employee.picture}`,
        }));
      }
    }
  }, [props.employee]);
