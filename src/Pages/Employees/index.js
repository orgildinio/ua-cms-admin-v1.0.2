...
const Employee = (props) => {
  ...
  const [columns, setColumns] = useState([
    ...
    },
    {
      ...
      render: (text, record) => {
        return record.positions.map((el) => <Tag color="blue"> {el} </Tag>);
      },
    },
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/employees/edit/${selectedRowKeys[0]}`);
    }
  };
    ...
    if (props.employees) {
      ...
      props.employees.length > 0 &&
        props.employees.map((el) => {
          ...
          el.degree = el[cookies.language] ? (
            el[cookies.language].degree
          ) : (
            <span className="red-color">
              {el[cookies.language === "eng" ? "mn" : "eng"].degree}
            </span>
          );
          ...
          el.positions = el.positions.map((el) =>
            el[cookies.language] ? (
              el[cookies.language].name
            ) : (
              <span>
    ...
    switch (modal) {
      ...
      case "edit": {
        if (selectedRowKeys && selectedRowKeys.length === 1) {
          props.history.replace("/employee/edit/" + selectedRowKeys[0]);
        } else {
          toastControl("error", "Нэг өгөгдөл сонгоно уу");
        }
        break;
          ...
          <div className="container-fluid">
            <div className="card datatable-card">
              <div className="card-header">
                <h3 className="card-title">Бүх ажилчид</h3>
              </div>
