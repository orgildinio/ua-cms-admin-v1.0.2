...
const News = (props) => {
  ...
  const [select, setSelect] = useState("status name link logo createAt");
  ...
  const init = () => {
    ...
    setSearchText(() => "");
    setChkBox(() => []);
    props.loadPartner(`select=${select}`);
  };

  const addClick = () => {
    props.history.push("/partners/add");
  };
  ...
  return (
    <Section>
      <MetaTags>
        <title> Хамтрагчид | WEBR Control Panel</title>
        <meta name="description" content="Хамтрагчид | WeBR control panel" />
        <meta property="og:title" content="Хамтрагчид | web control panel" />
      </MetaTags>
        ...
        <div className={css.PanelControl}>
          <div className="col-md-4">
            <div className={css.PanelTabelHeader}>
              ...
              >
                <i className="fas fa-plus-circle"></i> Хамтрагчид нэмэх{" "}
              </button>
              <button
                name="refresh"
                onClick={() => init()}
                      ...
                      <td>
                        {el.logo ? (
                          <div className="tableImgBox">
                            <img
                              src={`${myBase.cdnUrl}uploads/150x150/${el.logo}`}
                              className="tableImg"
                            />
                          </div>
                        ) : (
...
const mapStateToProps = (state) => {
  return {
    parents: state.partnerReducer.parents,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    loadPartner: (query) => dispatch(actions.loadParents(query)),
    loadPagination: (pageLast) => dispatch(actions.loadPagination(pageLast)),
    deleteMultParent: (ids) => dispatch(actions.deleteMultParent(ids)),
  };
};
