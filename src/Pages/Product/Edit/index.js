...
const Add = (props) => {
  ...
  useEffect(() => {
    ...
    let c = [];
    props.product.menu && props.product.menu.map((el) => c.push(el._id));
  ...
  const convertFromdata = () => {
    ...
    Object.keys(formData).map((index) => {
      if (index === "pictures" || index === "menu" || index === "oldPicture") {
  ...
  return (
    <Section>
      <MetaTags>
        <title> Бүтээгдэхүүний мэдээлэл засах | WEBR Control Panel</title>
        <meta
          name="description"
          content="Бүтээгдэхүүн мэдээлэл засах | WeBR control panel"
        />
        <meta
          property="og:title"
          content="Бүтээгдэхүүн мэдээлэл засах | web control panel"
        />
      </MetaTags>

      <PageTitle name="Бүтээгдэхүүн мэдээлэл засах" />
      <div className="row">
